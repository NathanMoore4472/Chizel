...
import chizelLogo from "../../icons/Chizel_banner.svg";
import {
  DndContext,
  DragOverlay,
  PointerSensor,
...
export default function AppLayout() {
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (activeData?.kind === "palette") {
        ...
        const targetId =
          overData?.kind === "container" ? overData.nodeId : ROOT_FRAME_ID;
        ...
        const x = Math.max(
          0,
          dropClientX - (containerRect?.left ?? 0) + scrollLeft,
        );
        const y = Math.max(
          0,
          dropClientY - (containerRect?.top ?? 0) + scrollTop,
        );
        ...
        const isAbsolute =
          targetDef?.childLayout?.(targetNode?.props ?? {}) === "absolute";
      ...
      // Tree → Tree reorder
      if (
        active.data.current?.kind === "tree" &&
        over?.data.current?.kind === "tree"
      ) {
  ...
  useEffect(() => {
    const handler = async (e: KeyboardEvent) => {
      ...
      if (mod && e.key === "s" && isTauri()) {
        e.preventDefault();
        const { tree, dataSources, currentFilePath, setCurrentFilePath } =
          useEditorStore.getState();
        const json = JSON.stringify({ tree, dataSources }, null, 2);
        const path = await saveProjectFile(
          json,
          e.shiftKey ? undefined : (currentFilePath ?? undefined),
        );
      ...
      if (mod && e.key === "o" && isTauri()) {
        ...
        if (!result) return;
        const { path, data } = result as {
          path: string;
          data: { tree: any; dataSources: any };
        };
        useEditorStore
          .getState()
  ...
  return (
    <DndContext
      sensors={sensors}
      collisionDetection={innermostContainer}
      ...
      <div className="flex h-screen w-screen overflow-hidden bg-editor-bg">
        ...
        <div className="absolute top-0 left-0 right-0 h-8 bg-editor-panel border-b border-editor-border flex items-center px-3 gap-3 z-50">
          <img
            src={chizelLogo}
            alt="Chizel"
            className="pl-1 h-5 w-auto grayscale invert-[0.8]"
          />
          <FileMenu />
        </div>
